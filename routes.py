"""
=============================================================================
ROUTES — WHERE EACH URL GOES (routes.py)
=============================================================================

WHAT THIS FILE DOES (in plain language):
----------------------------------------
When the browser (or the extension) asks the server for something, it uses a
URL. For example:
  - "GET /"              → "Give me the main webpage (index.html)"
  - "POST /chat/stream"  → "Send this conversation to the AI and stream the reply"
  - "GET /engagement/state" → "What is the current engagement score and any alert?"

This file defines every such URL and which Python function handles it. We group
URLs into "blueprints":
  - pages_bp   — The main page, dashboard redirect, static files (JS/CSS), favicon.
  - chat_bp    — Non-streaming and streaming chat with the AI.
  - config_bp  — All /config/* and /speech/token (so the frontend can get settings).
  - engagement_bp — Start/stop engagement, upload video, receive frames and
                    transcript, get current state (score, alerts, insight text).

The app (app.py) calls register_routes(app) to attach these blueprints so the
server knows where to send each request.

WHO CALLS THESE ROUTES:
-----------------------
  - The main webpage (index.html) and its JavaScript (e.g. app-main.js,
    engagement-detector.js) call these URLs.
  - The browser extension may also call them (e.g. when the app is in an iframe).
=============================================================================
"""
...
# App root for reliable paths (works from any working directory, including Docker)
...
from helpers import set_partner_frame_from_bytes, VideoSourceType, SIGNIFIER_KEYS
...
# -----------------------------------------------------------------------------
# BLUEPRINT: PAGES & STATIC FILES
# -----------------------------------------------------------------------------
# These routes serve the actual web page and assets (JS, CSS). No API logic
# here — just "send this file" or "redirect there".
# -----------------------------------------------------------------------------
...
def index():
    """
    Serve the main single-page app (index.html).

    When a user opens http://localhost:5000/ in the browser, the server sends
    the contents of index.html. That page then loads JavaScript that calls our
    other routes (chat, engagement, config).
    """
    try:
...
def dashboard():
    """
    Redirect /dashboard to the main app (/).

    We used to have a separate dashboard page; now the dashboard is built into
    the main page (left panel). This redirect keeps old links working.
    """
...
def static_files(filename):
    """Serve static files (JS, CSS, etc.) with cache headers for performance."""
    try:
...
# -----------------------------------------------------------------------------
# BLUEPRINT: CHAT (AI conversation)
# -----------------------------------------------------------------------------
# The frontend sends the user's message and (optionally) conversation history.
# We add engagement context to the last message so the AI "sees" the current
# meeting state, then call Azure Foundry and return the reply.
# -----------------------------------------------------------------------------
...
def chat():
    """
    Non-streaming chat: send one message, get one full reply.

    The client sends JSON with "message". We call the AI (Foundry) with that
    message and the system prompt, then return the full response in one go.
    Use this for simple request/response; for typing effect use /chat/stream.
    """
...
@config_bp.route("/config/openai", methods=["GET"])  # backward compatibility
@config_bp.route("/config/foundry", methods=["GET"])
def get_foundry_config():
    """Get Azure AI Foundry configuration (openai path is legacy alias)."""
    return jsonify({
...
def get_stt_tts_config():
    """Get STT configuration (locales, continuous conversation). Used for passing spoken context to Foundry for insights."""
    return jsonify({
...
# -----------------------------------------------------------------------------
# BLUEPRINT: ENGAGEMENT DETECTION
# -----------------------------------------------------------------------------
# The frontend sends video (or frames), transcript, and acoustic data. The
# backend runs the engagement detector (see detector.py) and exposes
# the current state and any alert/insight via GET /engagement/state. These
# routes are the bridge between the browser and the detector.
# -----------------------------------------------------------------------------
...
def start_engagement_detection():
    """
    Start the engagement detection pipeline from a video source.

    The client sends JSON: sourceType (webcam, file, stream, partner) and
    optionally sourcePath (for file/stream). We start the detector with that
    source; it then runs in a background thread and updates state. The client
    polls GET /engagement/state to get score, metrics, and alerts.
    """
    if not request.is_json:
        return jsonify({"error": "Request must be JSON"}), 400
    data = request.get_json(silent=True) or {}
...
def upload_video_file():
    ...
    try:
        ...
        upload_dir = _APP_ROOT / "uploads"
        upload_dir.mkdir(parents=True, exist_ok=True)
        filepath = upload_dir / file.filename
        file.save(str(filepath))
        return jsonify({"success": True, "filePath": str(filepath), "message": "File uploaded successfully"})
    except Exception as e:
...
def get_engagement_state():
    ...
    try:
        ...
        if sig_scores and isinstance(sig_scores, dict):
            sig_scores = {k: max(0.0, min(100.0, _safe_float(v))) for k, v in sig_scores.items()}
        else:
            sig_scores = {}
        # Ensure every signifier key is present for dashboard rows
        for key in SIGNIFIER_KEYS:
            sig_scores.setdefault(key, 0.0)
        ...
        score_val = _safe_float(state.score)
        ts_val = _safe_float(getattr(state, "timestamp", None), default=time.time())
        conf_val = _safe_float(getattr(state, "confidence", None))
        try:
...
def get_context_and_response():
    """Get last context sent to Azure AI Foundry and last AI response (for API consumers)."""
    store = engagement_api.get_context_store()
    return jsonify({
...
def record_response():
    """Record the full assistant reply after chat stream completion."""
    if not request.is_json:
        return jsonify({"error": "Request must be JSON"}), 400
    data = request.get_json(silent=True) or {}
...
def set_additional_context():
    """Store user-defined context for next batch sent to Azure AI Foundry."""
    if not request.is_json:
        return jsonify({"error": "Request must be JSON"}), 400
    data = request.get_json(silent=True) or {}
...
def context_push():
    """Build context bundle, call Azure AI Foundry, store context and response, return both."""
    try:
        full_context = engagement_api.build_engagement_context_bundle(None)
        store = engagement_api.get_context_store()
        store["last_context_sent"] = full_context
