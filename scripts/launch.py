...
"""
=============================================================================
DESKTOP LAUNCHER FOR BUSINESS MEETING COPILOT (scripts/launch.py)
=============================================================================

WHAT THIS FILE DOES (in plain language):
----------------------------------------
This is an optional desktop app that gives you a small window (using Tkinter)
to control the server and run tests without using the command line. Think of
it as a "control panel" for people who prefer clicking buttons.

  • START SERVER — Runs "python app.py" in the background so the web app is
    available at http://localhost:5000.
  • STOP SERVER — Stops the server process and clears port 5000.
  • RESTART SERVER — Stop then start again (on Windows can use restart.bat).
  • OPEN IN BROWSER — Opens http://localhost:5000 in your default browser.
  • RUN TESTS — Runs the test suite and shows pass/fail with details. If a
    test fails, it shows possible reasons (e.g. "pip install -r requirements.txt").

You do NOT need the launcher to run the project. You can always run
"python app.py" from the project root and open the URL in the browser. The
launcher is just a convenience.

HOW TO RUN:
-----------
  - From project root:  python scripts/launch.py
  - Or double-click:   scripts\\start.bat  (Windows)
  - Or from project root:  ./scripts/start.sh  (Mac/Linux)

The launcher expects to live in the "scripts" folder; it finds the project
root (parent of scripts/) so it can start app.py and run tests correctly.
=============================================================================
"""
...
# Paths and theme
# ---------------------------------------------------------------------------
# When launcher is in scripts/, project root is parent of scripts/; else project root is same as launcher dir.
_LAUNCHER_DIR = Path(__file__).resolve().parent
if _LAUNCHER_DIR.name == "scripts":
    PROJECT_ROOT = _LAUNCHER_DIR.parent
    SCRIPT_DIR = _LAUNCHER_DIR
else:
    PROJECT_ROOT = _LAUNCHER_DIR
    SCRIPT_DIR = _LAUNCHER_DIR
...
def run_tests_with_results(pattern: str | None = None) -> tuple[list[str], list, list]:
    ...
    suite = loader.discover(start_dir, pattern="test_*.py", top_level_dir=str(PROJECT_ROOT))
...
class ControlPanel:
    ...
    def _restart_server(self) -> None:
        if sys.platform == "win32":
            bat = self.script_dir / "restart.bat"
            if bat.exists():
