// ========================================================================
// INLINE ENGAGEMENT DASHBOARD (dashboard-inline.js)
// ========================================================================
//
// WHAT THIS MODULE DOES (in plain language):
// ------------------------------------------
// Drives the left-panel metrics on the main page (index.html): signifier bars,
// composite bars, and engagement score. It polls the backend for engagement state
// and updates the UI. Only runs when the inline dashboard container exists
// (main window, not the extension panel-only view).
//
// WHO USES THIS: index.html includes this script when the inline dashboard is
// present; it works alongside app-main.js and the engagement detector.
//
(function () {
    'use strict';
    ...
    var SIGNIFIER_LABELS = {
        g1_duchenne: 'Duchenne Smile Marker', g1_pupil_dilation: 'Pupil Dilation', g1_eyebrow_flash: 'Eyebrow Flash',
        ...
        g1_micro_smile: 'Micro Smile', g1_brow_raise_sustained: 'Brow Raise Sustained', g1_mouth_open_receptive: 'Mouth Open Receptive', g1_eye_widening: 'Eye Widening', g1_nod_intensity: 'Nodding Intensity',
        g2_look_up_lr: 'Looking Up (Left or Right)', g2_lip_pucker: 'Lip Pucker', g2_eye_squint: 'Eye Squinting', g2_thinking_brow: 'Thinking Brow',
        g2_stillness: 'Stillness', g2_gaze_shift_frequency: 'Gaze Shift Frequency', g2_mouth_tight_eval: 'Mouth Tight (Evaluative)',
        g3_contempt: 'Contempt', g3_nose_crinkle: 'Nose Crinkle', g3_lip_compression: 'Lip Compression', g3_eye_block: 'Eye Block',
        g3_jaw_clench: 'Jaw Clenching', g3_rapid_blink: 'Rapid Blinking', g3_gaze_aversion: 'Gaze Aversion', g3_no_nod: 'Absence of Nodding',
        ...
        g4_relaxed_exhale: 'Relaxed Exhale', g4_fixed_gaze: 'Fixed Gaze', g4_smile_transition: 'Smile Transition to Genuine', g4_mouth_relax: 'Mouth Relaxation', g4_smile_sustain: 'Sustained Smile'
    };
    ...
    var COMPOSITE_LABELS = {
        verbal_nonverbal_alignment: 'Words match demeanor', cognitive_load_multimodal: 'Information overload',
        rapport_engagement: 'Rapport with speaker', skepticism_objection_strength: 'Objection or pushback',
        decision_readiness_multimodal: 'Ready to decide', opportunity_strength: 'Closing opportunity',
        trust_rapport: 'Trust and rapport', disengagement_risk_multimodal: 'Attention slipping',
        confusion_multimodal: 'Confusion or uncertainty', tension_objection_multimodal: 'Tension or objection',
        loss_of_interest_multimodal: 'Losing interest', decision_plus_voice: 'Ready to close (with voice)',
        psychological_safety_proxy: 'Psychological safety', urgency_sensitivity: 'Urgency or timeline focus',
        skepticism_strength: 'Skepticism strength', enthusiasm_multimodal: 'Enthusiasm',
        hesitation_multimodal: 'Hesitation or hedging', authority_deferral: 'Deferring to authority',
        rapport_depth: 'Rapport depth', cognitive_overload_proxy: 'Cognitive overload'
    };
    // Order: meeting-critical first (decision, rapport, objection, confusion, attention), then alignment and dynamics
    var COMPOSITE_KEYS = ['decision_readiness_multimodal','rapport_engagement','skepticism_objection_strength','confusion_multimodal','disengagement_risk_multimodal','verbal_nonverbal_alignment','opportunity_strength','loss_of_interest_multimodal','tension_objection_multimodal','decision_plus_voice','trust_rapport','cognitive_load_multimodal','psychological_safety_proxy','enthusiasm_multimodal','hesitation_multimodal','urgency_sensitivity','skepticism_strength','rapport_depth','cognitive_overload_proxy','authority_deferral'];
    ...
    var BASIC_KEYS = ['attention','eyeContact','facialExpressiveness','headMovement','mouthActivity'];
    var BASIC_LABELS = { attention: 'Attention', eyeContact: 'Eye Contact', facialExpressiveness: 'Facial Expressiveness', headMovement: 'Head Movement', mouthActivity: 'Mouth Activity' };
    ...
    function updateEngagementState(data) {
        ...
        var card = document.getElementById('engagementCard');
        if (!card) return;
        var m = data.metrics || {};
        for (var i = 0; i < BASIC_KEYS.length; i++) {
            var k = BASIC_KEYS[i];
            ...
            var row = card.querySelector('[data-metric="' + k + '"]');
        ...
        for (var g = 0; g < SIGNIFIER_GROUPS.length; g++) {
            for (var j = 0; j < SIGNIFIER_GROUPS[g].keys.length; j++) {
                ...
                var num = v != null ? Number(v) : NaN;
                var pct = (!isNaN(num) && isFinite(num)) ? Math.max(0, Math.min(100, num)) : 0;
                var row = card.querySelector('[data-signifier="' + key + '"]');
                setBarAndValue(row, pct, (!isNaN(num) && isFinite(num)) ? Math.round(num).toString() : '—', barColor(pct));
            }
        }
        ...
        for (var c = 0; c < COMPOSITE_KEYS.length; c++) {
            ...
            var num = v != null ? Number(v) : NaN;
            var pct = (!isNaN(num) && isFinite(num)) ? Math.max(0, Math.min(100, num)) : 0;
            var row = card.querySelector('[data-composite="' + key + '"]');
            setBarAndValue(row, pct, (!isNaN(num) && isFinite(num)) ? Math.round(num).toString() : '—', barColor(pct));
        }
    ...
    pollEngagement();
    pollContext();
    setInterval(pollEngagement, 200);
