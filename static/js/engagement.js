/**
 * =============================================================================
 * ENGAGEMENT DETECTION MANAGER (engagement-detector.js)
 * =============================================================================
 *
 * WHAT THIS MODULE DOES (in plain language):
 * ------------------------------------------
 * This script runs in the browser and talks to the server to run "engagement
 * detection" on video. It:
 *
 *   - STARTS/STOPS detection — Sends POST /engagement/start with the chosen
 *     video source (webcam, partner, or file). The server then reads frames
 *     and computes the engagement score and alerts.
 *
 *   - POLLS STATE — Every few hundred milliseconds we call GET /engagement/state
 *     to get the current score, metrics, and any alert (with insight text). We
 *     update the UI (signifier panel, composite panel, etc.) and show popups
 *     when there is an alert.
 *
 *   - PARTNER VIDEO — When the user selects "Meeting Partner Video", we capture
 *     frames from the shared screen/tab and send them to POST /engagement/partner-frame
 *     so the server can analyze the other person's face. Feed audio is sent
 *     separately (by the insight session) for speech/transcript.
 *
 * This module uses VIDEO for visual cues; the insight session (app-main.js and
 * backend) uses AUDIO for aural cues. The backend combines both for B2B insights
 * and generates popup text via Azure AI Foundry.
 *
 * =============================================================================
 */
"use strict";
...
class EngagementDetector {
     ...
     * @param {number} options.pollInterval - Polling interval in milliseconds (default: 250; increase to reduce server load)
     */
    constructor(options = {}) {
    ...
    async stop() {
        try {
            ...
            if (this.compositeMetricsPanel) {
                this.compositeMetricsPanel.reset();
            }
    ...
    async pollEngagementState() {
        ...
        try {
            ...
            if (data && data.detectionStarted === false) {
                // Backend returns 200 with detectionStarted: false when not started
                return;
            }
            ...
            // Metric spike alert: show text popup
    ...
    _setMetricsPanelVisibility(detectionMethod) {
        ...
        var compositeEl = document.getElementById('compositeMetricsPanelContainer');
        ...
        if (compositeEl) compositeEl.style.display = isAzure ? 'none' : 'flex';
    }
    ...
    startPartnerFrameStream(stream) {
        ...
        const startSending = () => {
            ...
            sendFrame();
            this._partnerFrameIntervalId = setInterval(sendFrame, 33);   // ~30 fps (min target); backend can run up to 60 when possible
