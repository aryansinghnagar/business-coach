"""
Engagement State Detector — state-of-the-art psychology of business meetings.

Orchestrates real-time engagement from video: face detection (MediaPipe or Azure),
30 signifiers → 4 group means (G1–G4), composite metrics (facial + speech + acoustic),
overall score (0–100), spike alerts, and B2B opportunity detection. Alerts are consumed
by GET /engagement/state and turned into text-only popup via the insight generator.

Research basis for accurate engagement state detection and advice to capitalize on
opportunities:
- Engagement bands (VERY_LOW/LOW/MEDIUM/HIGH/VERY_HIGH) follow when-to-intervene vs.
  when-to-seek-commitment research (virtual/sales meeting engagement; Cialdini
  commitment/consistency; Edmondson psychological safety; receptivity windows).
- VERY_LOW/LOW → re-engage immediately (simplify, check understanding, reduce
  cognitive load; Kahneman, Sweller).
- MEDIUM → build interest, address concerns proactively, invite participation.
- HIGH/VERY_HIGH → capitalize: present proposals, seek commitments, advance to
  next steps (strike while commitment cues are high; Cialdini).
- Composites and acoustic tags (Scherer, Bachorowski, Ladd; vocal affect) refine
  state and surface decision readiness, confusion, resistance, and disengagement
  for targeted advice.

Negative opportunities use lower thresholds and shorter cooldowns so the host can
address concerns early; positive (closing, decision-ready) use higher thresholds
and longer cooldowns to surface strong capitalize moments without popup fatigue.

Pipeline: capture frame → detect faces, signifiers, composites → update state and callbacks.
"""
...
# --- Azure engagement metrics (inlined from utils.azure_engagement_metrics) ---
_AZURE_EMOTION_KEYS = [
...
class EngagementStateDetector:
    ...
    def _metrics_from_signifiers(
        ...
        """Build EngagementMetrics from signifier scores (lightweight path). See docs/DOCUMENTATION.md §16.
