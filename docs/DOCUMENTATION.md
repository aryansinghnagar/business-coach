...
15. [Composite Metrics (Detailed)](#15-composite-metrics-detailed)
16. [Metrics Reference (Formulas and Indices)](#16-metrics-reference-formulas-and-indices)
17. [Performance Optimizations](#17-performance-optimizations)
18. [Signifier Parameter Sweep](#18-signifier-parameter-sweep)
...
2. **Start the server:**
   ...
   - **Mac/Linux:** Run `./start.sh` or `python app.py`.
...
| **routes.py** | Blueprints defining every URL (pages, chat, config, engagement). |
| **engagement_detector.py** | Core logic: reads video, detects faces, computes engagement and alerts. |
| **launcher.py** | Optional desktop launcher (start/stop server, open browser, run tests). |
| **test_runner.py** | Runs the test suite. |
| **requirements.txt** | Python package list for `pip install`. |
| **docs/** | This file only (single consolidated documentation). |
...
| **backup/** | See README there; use version control for history. |
...
- **What it is:** Used for **speech-to-text (STT)** so the client’s spoken words are converted to text and sent to the backend for transcript storage and speech cue analysis (Azure Foundry). Avatar/TTS has been removed; insights are text-only.
- **Main settings:**
...
1. **Imports** the Flask app framework, CORS, compression, and **register_routes** from the **routes** package (all URL handlers), plus `config`.
2. **`create_app()`**  
...
### 5.1 launcher.py (optional)

**launcher.py** is an **optional desktop launcher** you can run with `python launcher.py`. It opens a **small window** (using Tkinter) that lets you:
...
- **GET /config/stt-tts**  
  Returns STT settings (locales, etc.). TTS/avatar config is no longer used for playback.

- **GET /config/avatar**  
  Returns avatar-related settings (retained for config compatibility; avatar speech is disabled).
...
- Avatar speech/TTS has been **removed**. Insights are text-only. The **/avatar/relay-token** route is no longer provided.
...
Route handlers live in **routes.py** and call **services** for detector, context, and request idle tracking (update_last_request, is_idle). The rest is reading request body, calling services, formatting JSON, and handling errors.

---

## 7. Engagement State Detector

The **EngagementStateDetector** (in **engagement_detector.py**) is the **core component** that turns **video (and optionally transcript/acoustic)** into a single **engagement state** and **alerts**. Everything else (API, insights, chat) depends on this.
...
- **EngagementStateDetector (class)**  
  ...
  - **(removed)** `_average_metrics()` was dead code; removed in simplification.  
...
So: **engagement_detector.py** ties together **video → faces → signifiers → groups → score + context + alerts**, and exposes that as **current state** and **pending alert** for the rest of the app.
...
- **What it does:** Wraps **Azure Speech Service** for **token** retrieval (STT only; no avatar/TTS).
...
### 9.9 Azure engagement metrics (in engagement_detector.py)

- **What it does:** **Engagement metrics from Azure Face API** (emotions, B2B composites, 0–100 score). Inlined in **engagement_detector**; used when detection method is Azure or unified.
...
### 9.12 utils/signifier_weights.py

- **What it does:** **Loads** the **signifier weights** (30 for signifiers, 4 for groups) from local file or uses built-in defaults. No external backend fetch. Provides a **weights_provider** callable for the signifier engine.
- **get_weights()**  
  Returns the current weights dict (signifier list, group list).  
- **load_weights()**  
  Loads from local path or applies built-in defaults.  
- **get_fusion_weights()**  
...
### 9.14 utils/detection_capability.py
...
- **build_config_response()**  
  Returns a dict with speech, foundry, cognitiveSearch, sttTts, avatar, systemPrompt, faceDetection, azureFaceApi, acoustic.
...
- **Insights / popups:** Area or overlay where **insight messages** appear (text only; no TTS).
...
- **avatar-chat-manager.js**  
  Handles **chat** UI and streaming responses (text-only). Avatar/TTS and relay token are no longer used.
...
- **extension/** — Single folder with browser-specific subfolders:
  - **extension/chrome/** — Chrome/Edge extension (manifest v3, icons, popup, sidepanel, background).
  - **extension/firefox/** — Firefox extension (manifest v2, similar structure).
...
- **tests/**  
  ...
  - **test_suite.py** (consolidated test suite at project root)  
    Tests for **helpers** (e.g. video handler, signifier engine, scorer, context generator).  
- **tests/fixtures/**  
...
Running **python test_runner.py** runs all of these and reports pass/fail. This helps ensure that changes to config, routes, detector, services, or helpers don’t break the pipeline.
...
- **run_tests.bat** (calls test_runner.py)  
  Runs **python test_runner.py** so you can run the test suite with one click.

These are **convenience** scripts; the real behavior is in **app.py**, **routes.py**, and **test_runner.py**.

---

## 15. Composite Metrics (Detailed)

Composite metrics combine **44 facial signifiers** (from **helpers.py**), **speech categories** (phrase cue detector), and **acoustic tags** (voice analyzer) into 0–100 scores. Two types: **multimodal composites** (weighted blends of G1–G4, speech, acoustic) and **condition-based composites** (specific signifier combinations, e.g. forward lean 60–80 AND eye contact ≥ 70).
...
## 16. Metrics Reference (Formulas and Indices)
...
**Basic metrics from signifiers** (engagement_detector._metrics_from_signifiers): attention = mean(duchenne, eye_contact, eyebrow_flash, pupil_dilation); eye_contact = g1_eye_contact; facial_expressiveness = mean(duchenne, parted_lips, smile_transition, softened_forehead); head_movement = mean(head_tilt, rhythmic_nodding, forward_lean); symmetry = g1_facial_symmetry; mouth_activity = g1_parted_lips.
...
Optional: run **python test_runner.py** with **WRITE_SIGNIFIER_SWEEP_DOC=1** to generate **docs/sweep_results.md** with current score ranges from the test suite.

---

## Summary

- **Business Meeting Copilot** is an **AI meeting coach**: it uses **video** (webcam or partner) and **audio/transcript** (STT → backend for speech cue analysis) to estimate **engagement** and **B2B opportunities**, and to produce **coaching insights** (text-only popups and chat). Avatar speech/TTS has been removed.
