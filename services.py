"""
=============================================================================
SERVICES — BACKEND LOGIC FOR AI, SPEECH, FACE, AND ENGAGEMENT (services.py)
=============================================================================

WHAT THIS FILE DOES (in plain language):
----------------------------------------
This is the "brain" of the server. It does not serve web pages; it does the
actual work when the frontend or routes ask for something:

  1. ACOUSTIC CONTEXT — The browser sends short summaries of voice (loudness,
     pitch, tone). We store them and turn them into tags (e.g. "uncertainty",
     "tension") that the engagement system and insight generator use.

  2. AZURE FACE API — When we use cloud face detection, we send images to
     Microsoft's Face API and get back faces with emotions (anger, happiness,
     etc.) and optional landmarks.

  3. AZURE FOUNDRY — We send conversation + context to the AI and get back
     chat or insight text (no voice playback; text only).

  4. AZURE SPEECH — We create tokens so the *browser* can use Speech-to-Text
     without seeing our key. We do not do STT in this file; the browser does.

  5. INSIGHT GENERATOR — When there is an "alert" (e.g. a spike in engagement
     or a phrase like "I'm not sure"), we build a short text suggestion using
     the current state and optionally the AI (Foundry).

  6. ENGAGEMENT API — Start/stop the detector (in detector.py),
     get current state, build context bundles for the AI. The detector runs
     in its own thread; this module exposes it and its state to the routes.

WHO USES THIS:
--------------
  - routes.py calls functions here (e.g. get_foundry_service(), get_engagement_state).
  - detector.py calls get_recent_speech_tags, get_recent_acoustic_*,
    and uses the insight generator for alert text.
=============================================================================
"""
...
# =============================================================================
# The frontend (acoustic-analyzer.js) sends "windows" of voice features:
# loudness, pitch, contour (rising/falling), variability, tone. We store
# recent windows and interpret them into a short text summary and tags
# (e.g. acoustic_uncertainty, acoustic_tension). Those tags are used by
# the engagement composites and the insight generator.
# =============================================================================

# Thresholds used when turning numbers into tags (see interpret_acoustic_windows below).
LOUDNESS_LOW = 0.15
LOUDNESS_HIGH = 0.6
...
def interpret_acoustic_windows(windows: List[Dict[str, Any]]) -> Tuple[str, List[str]]:
    """
    Turn a list of acoustic "windows" (loudness, pitch, contour, etc.) into
    a short natural-language summary and a list of tags.

    What are "windows"? The frontend sends chunks of voice data (e.g. every
    1.5 seconds when speech is active). Each chunk is one window. We look at
    recent windows and decide things like: "low energy + flat pitch → possible
    disengagement" or "high pitch variability → uncertainty". We return a
    sentence or two (summary) and tag names (e.g. acoustic_uncertainty) that
    other code can use.

    Returns:
        (summary_string, list_of_tag_names)
    """
    if not windows:
        return "", []
...
# Bounded store for acoustic windows (avoids unbounded memory growth).
...
def append_acoustic_windows(windows: List[Dict[str, Any]]) -> None:
    """
    Add one or more "acoustic windows" to the store (called by the route that
    receives POST /engagement/acoustic-context). Each window is a dict with
    loudness_norm, pitch_hz, pitch_contour, pitch_variability, tone_proxy,
    voicing_confidence, speech_active, etc. We keep a bounded list (see
    ACOUSTIC_MAX_WINDOWS); older windows drop off. Thread-safe.
    """
    if not windows:
        return
    with _acoustic_lock:
...
def get_recent_acoustic_context() -> str:
    """
    Return a short sentence or two summarizing recent voice (e.g. "Low vocal
    energy and relatively flat pitch suggest possible disengagement..."). Used
    when building context for the AI. Thread-safe.
    """
    with _acoustic_lock:
...
def get_recent_acoustic_tags() -> List[str]:
    """
    Return the list of acoustic tag names from recent windows (e.g.
    acoustic_uncertainty, acoustic_tension). Used by the engagement detector
    and composites. Thread-safe.
    """
    with _acoustic_lock:
        snap = list(_acoustic_windows)
...
def get_recent_acoustic_tags_and_negative_strength() -> Tuple[List[str], float]:
    """
    Return both the list of acoustic tags and a 0–1 "negative strength" in one
    lock and one interpret pass. Negative strength is derived from how many
    negative tags (e.g. disengagement_risk, tension) appear; used by composites.
    Thread-safe.
    """
    with _acoustic_lock:
        snap = list(_acoustic_windows)
...
# =============================================================================
# 4. Azure Speech (STT token for client speech recognition)
# =============================================================================

class AzureSpeechService:
    """Azure Speech Service: token for speech recognition (STT) only. Spoken context is passed to Foundry for generating intelligent insights."""
    ...
    def get_speech_token(self) -> Dict[str, str]:
        """Get an access token for Azure Speech Service (STT). Raises ValueError if not configured or 401."""
        ...
        try:
            ...
            raise requests.RequestException(f"Failed to get speech token: {str(e)}")
...
def get_insight_weights() -> Dict[str, Any]:
    """Return in-code insight weights (prompt_suffix, max_length, opportunity_thresholds)."""
...
# Bounded store for recent speech tags (avoids unbounded memory growth).
...
# Used by /api/engagement/context-and-response and /api/context-push (and chat stream).
...
def get_detector():
    """
    Return the current engagement detector instance (or None if not started).
    The detector runs in a background thread and holds the current state;
    routes and build_engagement_context_bundle use this to get state.
    """
...
def get_context_store() -> Dict[str, Any]:
    """
    Return the shared context store (dict). Holds things like last_context_sent,
    last_response, pending_user_context. Used by routes and the frontend to
    remember what was last sent to the AI and what the user added.
    """
...
def get_context_generator() -> ContextGenerator:
    """
    Return the shared ContextGenerator (creates on first use). It turns
    engagement state and optional text into natural-language context strings
    for the AI (e.g. "no face", or a summary of face + composites + opportunities).
    """
...
def build_engagement_context_bundle(additional_context: Optional[str] = None) -> str:
    """
    Build the full context string we send to the AI: current engagement state
    (or "no face" message), recent acoustic summary, acoustic tags, composite
    metrics, and any pending user-provided context. Used when the user sends
    a message or when we need a snapshot for an insight.
    """
...
def start_detection(source_type, source_path=None):
    """
    Start the engagement detector with the given video source. Creates a new
    EngagementStateDetector, starts it (which opens the video and begins the
    detection loop in a thread), and stores it as the global engagement_detector.
    Called by POST /engagement/start. Returns (True, info_dict) on success,
    (False, error_message) on failure.
    """
    global engagement_detector
    from detector import EngagementStateDetector
...
def stop_detection():
    """
    Stop the engagement detector and clear related state (transcript, speech
    tags, aural alert, opportunity state, acoustic context). Called by
