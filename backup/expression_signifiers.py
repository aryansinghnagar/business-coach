...
class ExpressionSignifierEngine:
    ...
    def _g3_rapid_blink(self) -> float:
        ...
        # Continuous in b: normal (0–3) -> 6–20; elevated (4–5) -> 48–78; high (6+) -> 82+
        if b >= 6:
            return 82.0 + min(18.0, (b - 6) * 6.0)
        if b >= 4:
            return 48.0 + (b - 4) * 15.0
        return 6.0 + b * 4.0  # 6, 10, 14, 18 for b=0,1,2,3
    ...
    def _g3_gaze_aversion(self, cur: dict, buf: list) -> float:
        ...
        # Continuous: sustained + deviation drive score; floor varies smoothly with total_dev
        if sustained_aversion >= 8 and total_dev > 18.0:
            return 55.0 + min(40.0, (total_dev - 18.0) / 24.0 * 40.0)
        if sustained_aversion >= 6 and total_dev > 14.0:
            return 38.0 + min(22.0, sustained_aversion * 2.5)
        if sustained_aversion >= 4 and total_dev > 20.0:
    ...
    def _g3_no_nod(self, buf: list) -> float:
        ...
        # Continuous: very still -> high; more movement -> lower score with smooth ramp
        ...
        if pitch_std < 1.0 and zero_crossings < 1:
            return 45.0 + (1.0 - pitch_std) * 10.0
        # Normal movement: smooth decay 12–45 as pitch_std increases
        return 12.0 + max(0.0, (2.0 - pitch_std)) / 2.0 * 33.0
