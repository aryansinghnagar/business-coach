"""
=============================================================================
HELPERS — SHARED LOGIC FOR VIDEO, FACES, SCORING, AND INSIGHTS (helpers.py)
=============================================================================

WHAT THIS FILE DOES (in plain language):
----------------------------------------
This is a large "toolbox" used by the engagement detector and the server. It
does NOT run the server and does NOT talk to Azure directly. It provides:

  1. VIDEO SOURCE — Read frames from webcam, a video file, a stream URL, or
     "partner" (frames sent by the browser). The detector uses this to get
     one frame at a time.

  2. FACE DETECTION — Abstract interface so the rest of the app does not care
     whether we use MediaPipe (local) or Azure Face (cloud). Implementations
     return faces with landmarks and (for Azure) emotions.

  3. EXPRESSION SIGNIFIERS — From face landmarks (and optional Azure emotions),
     compute 42 scores (e.g. eye contact, smile, gaze aversion). These feed
     into the engagement score and alerts.

  4. ENGAGEMENT SCORER — Turn the 42 signifiers (and optional Azure metrics)
     into one 0–100 score and a level (e.g. LOW, HIGH).

  5. CONTEXT GENERATOR — Turn the current state into human-readable text
     (summary, indicators, suggested actions) for the AI and the UI.

  6. ENGAGEMENT COMPOSITES — Combine face + speech + voice into metrics like
     "decision readiness" or "confusion" (used for alerts and insights).

  7. B2B OPPORTUNITY DETECTOR — Detect moments like "closing window" or
     "confusion" from group means, speech tags, and acoustic tags.

  8. WEIGHTS & CAPABILITY — Signifier/fusion weights (in-code only) and
     device capability (e.g. recommend MediaPipe vs Azure based on CPU/RAM).

WHO USES THIS:
--------------
  - detector.py imports almost everything here (video, faces,
    signifiers, scorer, context, composites, opportunities, weights).
  - routes.py and services.py use helpers like set_partner_frame_from_bytes,
    build_config_response (in config.py), etc.
=============================================================================
"""
...
# =============================================================================
# 1. VIDEO SOURCE — Getting frames from webcam, file, stream, or "partner"
# =============================================================================
# The engagement detector needs one frame at a time. For webcam/file/stream we
# use OpenCV (cv2). For "partner" (Meeting Partner Video), the browser sends
# frames via POST /engagement/partner-frame; we store the latest one here and
# the VideoSourceHandler reads it when source_type is PARTNER.
# -----------------------------------------------------------------------------
# Shared state for partner source: the most recent frame received from the browser.
# -----------------------------------------------------------------------------
...
def set_partner_frame(frame_bgr: Optional[np.ndarray]) -> None:
    """
    Store the latest frame from the browser (partner source). Called when the
    server receives a POST /engagement/partner-frame with image data. Thread-safe.
    """
...
def get_partner_frame() -> Optional[np.ndarray]:
    """
    Return a copy of the latest partner frame, or None if none has been set.
    Does not clear the stored frame so the next read gets the same (or newer) frame.
    Thread-safe.
    """
    with _partner_frame_lock:
        out = _partner_frame
...
class VideoSourceType(Enum):
    """
    The four ways we can get video for engagement detection:
      WEBCAM — Built-in or USB camera.
      FILE   — A video file on disk (path required).
      STREAM — A URL or device that streams video (path required).
      PARTNER — Frames sent by the browser (e.g. shared tab/window). No OpenCV;
                we read from the shared partner frame set by set_partner_frame.
    """
    WEBCAM = "webcam"
    FILE = "file"
    STREAM = "stream"
...
class VideoSourceHandler:
    """
    Single place to get video frames for the engagement detector.

    No matter whether the user chose webcam, a file, a stream, or "partner"
    (browser-sent frames), we call initialize_source() once, then read_frame()
    in a loop. For webcam/file/stream we use OpenCV (cv2.VideoCapture). For
    partner we read from the shared _partner_frame set by the route when it
    receives POST /engagement/partner-frame. The detector does not need to
    know which source is active.
    """

    def __init__(self):
        """Create a handler; call initialize_source() before read_frame()."""
    ...
    def initialize_source(
        ...
        """
        Open the chosen video source so we can call read_frame() later.

        WEBCAM: opens camera 0 (or 1, 2) at 720p, 30 or 60 fps. FILE: opens
        the file at source_path. STREAM: opens the URL or stream at source_path.
        PARTNER: no OpenCV; we will read from get_partner_frame() when the
        browser sends frames. Returns True if the source is ready, False otherwise.
        """
        self.release()
...
# Human-readable labels for composite metrics (business-meeting focused; used in context and UI)
COMPOSITE_LABELS: Dict[str, str] = {
    # Multimodal — alignment, load, rapport, objections, decisions
    ...
    # Facial condition-based (meeting-relevant)
    "topic_interest_facial": "Leaning in (topic interest)",
    "active_listening_facial": "Active listening",
    "agreement_signals_facial": "Agreement / buy-in signals",
    "evaluating_thinking_facial": "Evaluating or thinking",
    "cognitive_processing_facial": "Processing information",
    "resistance_cluster_facial": "Resistance (contempt or aversion)",
    "receptivity_facial": "Receptivity to message",
    "withdrawal_facial": "Withdrawal (pulling back)",
    "disagreement_facial": "Disagreement signals",
    "closing_window_facial": "Closing window (ready to agree)",
    "passive_listening_facial": "Passive listening",
    "trust_openness_facial": "Trust and openness",
    "curious_engaged_facial": "Curious and engaged",
}
...
def _get_engagement_level():
    ...
    if _EngagementLevel is None:
        from detector import EngagementLevel as EL
        _EngagementLevel = EL
...
class ContextGenerator:
    ...
    # Backward compatibility alias (legacy OpenAI naming)
...
# All 40 signifier keys (face-only; chin stroke and mouth cover removed—required hand detection).
# Original 30 + research-based additions (Waller 2024, Ekman FACS, Edmondson, B2B negotiation).
SIGNIFIER_KEYS: List[str] = [
    ...
    "g2_look_up_lr", "g2_lip_pucker", "g2_eye_squint", "g2_thinking_brow",
    "g2_stillness", "g2_gaze_shift_frequency", "g2_mouth_tight_eval",
    "g3_contempt", "g3_nose_crinkle", "g3_lip_compression", "g3_eye_block", "g3_jaw_clench",
    "g3_rapid_blink", "g3_gaze_aversion", "g3_no_nod", "g3_narrowed_pupils",
...
_DEFAULT_SIGNIFIER_PARAMS: Dict[str, Any] = {
    ...
    "gaze_aversion_s6": 6,
    "gaze_aversion_s4": 4,
    "duchenne_au6_lo": 0.68,
    "duchenne_au6_hi": 1.0,
    "duchenne_au12_threshold": 0.002,
    ...
    "g2_look_up_pitch_deg": 3.0,
    "g2_look_up_yaw_deg": 5.0,
    "g2_eye_squint_ratio_lo": 0.75,
    "g2_eye_squint_ratio_hi": 0.98,
    "g2_thinking_brow_min_rel": 0.020,
    "g2_stillness_very_low_mov": 0.00008,
    "g2_lip_pucker_narrow": 0.30,
...
class ExpressionSignifierEngine:
    """42 signifier scores (0-100) from face landmarks; temporal buffer for blinks, nodding, stillness."""
    ...
    def __init__(
        ...
        # High sensitivity: shorter temporal windows so metrics react to micro-changes (config.HIGH_SENSITIVITY_METRICS)
        self._high_sensitivity: bool = getattr(config, "HIGH_SENSITIVITY_METRICS", True)
        self._win = (lambda n: max(2, n // 2)) if self._high_sensitivity else (lambda n: n)
        # Full range: stretch scores so they span 0–100 instead of hovering near 50 (config.FULL_RANGE_METRICS)
        self._full_range_metrics: bool = getattr(config, "FULL_RANGE_METRICS", True)
        self._range_stretch_gain: float = 1.85  # 50 stays 50; 30->13, 70->87; makes small changes more visible
        if self._high_sensitivity:
            self._params["lip_compression_sustained_min"] = 2
            self._params["lip_compression_sustained_all_min"] = 2
            self._params["narrowed_pupils_sustained_min"] = 1
        ...
        # Brow raise sustained: rest baseline (updated only when brow not raised) so score stays high while held
        self._brow_rest_baseline: float = 0.0
        # Brow lowered (g2): rest baseline for brow-eye distance (updated only when brow not lowered)
        self._brow_lowered_rest_baseline: float = 0.0
    ...
    def reset(self) -> None:
        ...
        self._brow_rest_baseline = 0.0
        self._brow_lowered_rest_baseline = 0.0
    ...
    def update(
        ...
        # Mouth corner lift ratio (for micro-smile baseline: compare current to recent median)
        mouth_corner_lift_ratio = 0.0
        if len(mouth_pts) >= 6:
            ly_m = float(mouth_pts[0, 1])
            ry_m = float(mouth_pts[5, 1])
            upper_y_m = float(np.mean(mouth_pts[1:5, 1]))
            left_l = upper_y_m - ly_m
            right_l = upper_y_m - ry_m
            cl = max(left_l, right_l) if (left_l > 0 or right_l > 0) else (left_l + right_l) / 2.0
            mouth_corner_lift_ratio = cl / max(face_scale, 1e-6)

        # Brow variance and evenness (for softened-forehead baseline-relative scoring)
        brow_variance_norm = 0.0
        brow_evenness = 0.8
        if len(lb) >= 2 and len(rb) >= 2:
            brow_pts = np.vstack([lb[:, :2], rb[:, :2]])
            v = float(np.var(brow_pts))
            fsq = max(face_scale * face_scale, 1e-6)
            brow_variance_norm = v / fsq
            li_y = float(lb[np.argmin(lb[:, 0]), 1])
            lo_y = float(lb[np.argmax(lb[:, 0]), 1])
            ri_y = float(rb[np.argmin(rb[:, 0]), 1])
            ro_y = float(rb[np.argmax(rb[:, 0]), 1])
            diff_n = (abs(li_y - lo_y) + abs(ri_y - ro_y)) / max(face_scale * 0.15, 1e-6)
            brow_evenness = max(0.0, 1.0 - min(1.0, diff_n))
        ...
        snap = {
            ...
            "mouth_corner_lift_ratio": mouth_corner_lift_ratio,
            "brow_variance_norm": brow_variance_norm,
            "brow_evenness": brow_evenness,
        }
        self._buf.append(snap)
        self._prev_landmarks = lm.copy()
        # Brow rest baseline: update only when brow is at rest (not raised) so sustained-raise score stays high
        cur_brow = (eyebrow_l + eyebrow_r) / 2.0
        if len(self._buf) >= 3:
            recent_h = [(b.get("eyebrow_l", 0) + b.get("eyebrow_r", 0)) / 2.0 for b in list(self._buf)[-12:]]
            median_h = float(np.median(recent_h))
            if cur_brow <= median_h * 1.03:  # at or below recent median => at rest
                if self._brow_rest_baseline <= 0:
                    self._brow_rest_baseline = cur_brow
                else:
                    self._brow_rest_baseline = 0.92 * self._brow_rest_baseline + 0.08 * cur_brow
        elif self._brow_rest_baseline <= 0:
            self._brow_rest_baseline = cur_brow
        # Brow lowered (g2) rest baseline: update only when brow is not lowered (brow-eye distance at or above low percentile)
        fs = max(face_scale, 20.0)
        cur_brow_ht_norm = (eyebrow_l + eyebrow_r) / 2.0 / fs
        if len(self._buf) >= 3:
            recent_ht_norm = [(b.get("eyebrow_l", 0) + b.get("eyebrow_r", 0)) / 2.0 / max(b.get("face_scale", 50.0), 20.0) for b in list(self._buf)[-12:]]
            p25 = float(np.percentile(recent_ht_norm, 25.0))
            if cur_brow_ht_norm >= p25 * 0.98:  # at rest (not lowered)
                if self._brow_lowered_rest_baseline <= 0:
                    self._brow_lowered_rest_baseline = cur_brow_ht_norm
                else:
                    self._brow_lowered_rest_baseline = 0.92 * self._brow_lowered_rest_baseline + 0.08 * cur_brow_ht_norm
        elif self._brow_lowered_rest_baseline <= 0:
            self._brow_lowered_rest_baseline = cur_brow_ht_norm
        # Frame counter for diagnostic logging (Phase C1)
        self._diag_frame_count = getattr(self, "_diag_frame_count", 0) + 1

    def _buf_tail(self, n: int) -> list:
        """Return last n buffer entries as a list (avoids copying full buffer)."""
        if n <= 0 or not self._buf:
            return []
        return list(itertools.islice(self._buf, max(0, len(self._buf) - n), None))
    ...
    def get_all_scores(self) -> Dict[str, float]:
        ...
        shp = self._shape
        buf = self._buf  # Use deque directly; _g* use self._buf_tail(n) for windows
        ...
        # Full-range stretch: spread scores toward 0 and 100 so metrics don't hover around the middle
        if self._full_range_metrics:
            gain = self._range_stretch_gain
            for k in list(out.keys()):
                v = float(out[k])
                if np.isfinite(v):
                    out[k] = 50.0 + (v - 50.0) * gain
        ...
        try:
            ...
            if getattr(config, "ENGAGEMENT_DIAGNOSTIC_LOGGING", False):
                ...
                if interval and fc % interval == 0:
                    cur = self._buf[-1] if self._buf else {}
                    diag = {
                        "raw": {
                            "ear": cur.get("ear"),
                            "mar": cur.get("mar"),
    ...
    def get_group_means(self, scores: Optional[Dict[str, float]] = None, W: Optional[Dict[str, List[float]]] = None) -> Dict[str, float]:
        ...
        g1k = ["g1_duchenne", "g1_pupil_dilation", "g1_eyebrow_flash", "g1_eye_contact", "g1_head_tilt",
               "g1_forward_lean", "g1_facial_symmetry", "g1_rhythmic_nodding", "g1_parted_lips", "g1_softened_forehead",
               "g1_micro_smile", "g1_brow_raise_sustained", "g1_mouth_open_receptive", "g1_eye_widening", "g1_nod_intensity"]
        g2k = ["g2_look_up_lr", "g2_lip_pucker", "g2_eye_squint", "g2_thinking_brow",
               "g2_stillness", "g2_gaze_shift_frequency", "g2_mouth_tight_eval"]
    ...
    def _g1_duchenne(self, lm: np.ndarray, cur: dict, buf: list) -> float:
        ...
        Tuned to be sensitive: small smile and subtle squinch can reach high scores (70–100).
        """
        ear = cur.get("ear", 0.2)
        ...
        # === AU 12: Lip Corner Pull (0–1) — more sensitive threshold and ramp ===
        ...
        AU12_threshold = float(self._params.get("duchenne_au12_threshold", 0.002))
        au12_intensity = 0.0
        if lift_ratio > AU12_threshold:
            # Gentler ramp so small smiles get 0.5+; full smile by ~0.02 lift_ratio
            au12_intensity = min(1.0, (lift_ratio - AU12_threshold) / 0.018)

        # === AU 6: Eye squinch — EAR drops when cheeks raise; linear sensitivity to narrowing ===
        au6_intensity = 0.0
        if baseline_ear > 0.05:
            ear_ratio = ear / baseline_ear
            au6_lo = float(self._params.get("duchenne_au6_lo", 0.68))
            au6_hi = float(self._params.get("duchenne_au6_hi", 1.0))
            # More narrowing (lower ratio) = higher squinch; full intensity when ratio <= au6_lo
            if ear_ratio <= au6_hi:
                span = au6_hi - au6_lo
                if span > 0.01:
                    au6_intensity = min(1.0, max(0.0, (au6_hi - ear_ratio) / span))
                else:
                    au6_intensity = 1.0 if ear_ratio <= au6_lo else 0.0

        # === Combine: higher weights so smile + squinch reach 75–100 easily ===
        corner_contrib = au12_intensity * 38.0
        squinch_contrib = au6_intensity * 38.0
        synergy = 0.0
        if au12_intensity > 0.15 and au6_intensity > 0.15:
            synergy = min(24.0, au12_intensity * au6_intensity * 40.0)
        raw_score = 18.0 + corner_contrib + squinch_contrib + synergy
    ...
    def _g1_pupil_dilation(self, cur: dict, buf: list) -> float:
        ...
        WHY IT CO-OCCURS WITH EYEBROW FLASH: Both use the same physical motion. Raising
        the brows (eyebrow flash) pulls the lid/skin up and increases eye contour area, so
        area/base rises and "dilation" scores high. We debias by discounting dilation when
        the brow is currently raised, so the two metrics don't double-count one gesture.
        
        FALSE POSITIVE REDUCTION: Median of recent eye-area (exclude blinks); debias when
        brow raised; sustained change from baseline.
        """
        ...
        if len(buf) >= 3:
            recent_areas = [b.get("eye_area", area) for b in self._buf_tail(self._win(4))
        ...
        # Steeper bands so small changes in eye openness produce visible score movement (full 0-100 span)
        if r >= 1.12:
            score = 68.0 + min(30.0, (r - 1.12) * 280.0)  # up to ~98
        elif r >= 1.06:
            score = 52.0 + (r - 1.06) / 0.06 * 28.0
        elif r >= 1.02:
            score = 38.0 + (r - 1.02) / 0.04 * 22.0
        elif r >= 0.98:
            score = 28.0 + (r - 0.98) / 0.04 * 18.0  # near baseline = mid-low
        elif r >= 0.92:
            score = 15.0 + (r - 0.92) / 0.06 * 18.0
        else:
            score = max(0.0, 8.0 + (r - 0.88) * 35.0)
        
        # Debiasing: when brow is raised (same gesture that drives eyebrow flash), part of
        # the eye-area increase is from the raise, not true dilation — discount to avoid double-counting
        if len(buf) >= 6:
            heights = [(b.get("eyebrow_l", 0) + b.get("eyebrow_r", 0)) / 2 for b in self._buf_tail(6)]
            baseline_brow = float(np.median(heights[:3]))
            cur_brow = float(np.mean(heights[-2:]))
            face_scale = buf[-1].get("face_scale", 50.0)
            if cur_brow > baseline_brow + face_scale * 0.022:
                score = 45.0 + (score - 45.0) * 0.55  # pull toward mid when brow raised
    ...
    def _g1_eyebrow_flash(self, buf: list) -> float:
        ...
        Scored from brow height (eyebrow_l + eyebrow_r) vs baseline. Same brow raise
        also increases eye contour area, so pupil dilation (eye-area proxy) often fires
        with it; pupil_dilation debiases when brow is raised to reduce double-counting.
        ...
        if raised and returning:
            ...
            return 62.0 + min(36.0, flash_magnitude * 110.0)  # up to ~98
        elif raised and cur > baseline + threshold * 0.6:  # Still raised
            raise_magnitude = (cur - baseline) / max(face_scale * 0.12, 1e-6)
            return 45.0 + min(35.0, raise_magnitude * 65.0)
        # Small sustained raise (not a flash, but mild interest)
        if cur > baseline + threshold * 0.6:
            return 42.0 + min(18.0, (cur - baseline) / max(face_scale * 0.08, 1e-6) * 45.0)
        return 32.0  # no raise → low so metric can span full range
    ...
    def _g1_eye_contact(self, cur: dict, w: int, h: int, buf: list) -> float:
        ...
        recent_frames = self._buf_tail(win_eye) if len(buf) >= win_eye else self._buf_tail(len(buf))
    ...
    def _g1_head_tilt(self, cur: dict, buf: list) -> float:
        ...
        if len(buf) >= 4:
            recent_rolls = [abs(float(b.get("roll", 0))) for b in self._buf_tail(self._win(4))]
            roll = float(np.median(recent_rolls))
        
        # No tilt (< 3.5°): neutral/passive → low so metric can span 0-100
        if roll < 3.5:
            return 28.0
        # Subtle tilt (3.5-7°): onset of interest
        if 3.5 <= roll < 7.0:
            return 35.0 + (roll - 3.5) / 3.5 * 22.0  # 35->57
        # Optimal band (7-18°): engagement/curiosity peak (Davidenko: 11° meaningful)
        if 7.0 <= roll <= 18.0:
            peak = 11.0
            dist = abs(roll - peak)
            return 62.0 + (1.0 - min(1.0, dist / 6.0)) * 33.0  # 62-95, peak at 11°
        # Strong tilt (18-28°): clear engagement, ramp down
        if 18.0 < roll <= 28.0:
            return 58.0 - (roll - 18.0) * 1.8  # 58->40
        # Very strong (28-40°): possible confusion; low
        if 28.0 < roll <= 40.0:
            return 38.0 - (roll - 28.0) * 0.6  # 38->31
    ...
    def _g1_forward_lean(self, cur: dict, buf: list) -> float:
        ...
        if len(buf) >= 4:
            recent_z = [b.get("face_z", z) for b in self._buf_tail(self._win(4))]
            z = float(np.median(recent_z))
        
        # z < baseline = lean toward camera; smaller ratio = stronger lean
        if z < base * 0.98:
            return 32.0 + min(66.0, (1.0 - z / base) * 220.0)  # 32-98 for full range
        return 32.0
    ...
    def _g1_facial_symmetry(self, lm: np.ndarray, buf: list) -> float:
        ...
        # Convert error to score: high symmetry (low error) = high score, low symmetry (high error) = low score.
        # Left/right convention: we compare left-feature to mirrored-right; if landmark "left" is
        # image-left (viewer's left), then mirrored-right falls on image-right and the pair is
        # reversed from geometric symmetry — so we treat total_error as "symmetry" and invert.
        # Ensure: at-rest (symmetric) -> high score; asymmetric -> low score.
        if total_error < 0.02:
        ...
        # If observed behavior is inverted (low at rest, high when asymmetric), flip so higher = more symmetric.
        return float(max(0.0, min(100.0, 100.0 - symmetry_score)))
    ...
    def _g1_micro_smile(self, lm: np.ndarray, cur: dict, buf: list) -> float:
        ...
        Baseline = median of recent lift_ratio (from buffer) so flat expression gives low score.
        Tuned to be more sensitive to small changes in facial expression.
        """
        mouth_pts = _safe(lm, MOUTH)
        if len(mouth_pts) < 6:
            return 30.0
        ...
        left_lift = upper_y - ly
        right_lift = upper_y - ry
        corner_lift = max(left_lift, right_lift) if (left_lift > 0 or right_lift > 0) else (left_lift + right_lift) / 2.0
        lift_ratio = corner_lift / face_scale
        # Shorter baseline window so small deviations from recent neutral are detected more easily
        win = 12
        if len(buf) >= win:
            recent = [b.get("mouth_corner_lift_ratio", lift_ratio) for b in self._buf_tail(win)]
            baseline = float(np.median(recent))
        else:
            baseline = lift_ratio
        effective = max(0.0, lift_ratio - baseline)
        # Lower thresholds and higher gain so small lifts score meaningfully; cap at 72 to avoid saturation
        if effective >= 0.007:
            return min(72.0, 48.0 + min(24.0, (effective - 0.007) * 1200.0))
        if effective >= 0.004:
            return 40.0 + (effective - 0.004) / 0.003 * 20.0
        if effective >= 0.002:
            return 34.0 + (effective - 0.002) / 0.002 * 12.0
        if effective >= 0.0008:
            return 30.0 + (effective - 0.0008) / 0.0012 * 8.0
        if effective >= 0.0003:
            return 28.0 + (effective - 0.0003) / 0.0005 * 4.0
        return 26.0

    def _g1_brow_raise_sustained(self, buf: list) -> float:
        """
        Sustained brow raise = interest/curiosity (FACS AU2). Score stays high while
        brow is kept raised. Uses a rest baseline (updated only when brow is not raised)
        so current elevated state is compared to the person's rest level, not to recent
        frames that are already raised.
        """
        if len(buf) < 2:
            return 35.0
        # Current brow = mean of last few frames (stable while sustained)
        recent = self._buf_tail(4) if len(buf) >= 4 else self._buf_tail(len(buf))
        heights = [(b.get("eyebrow_l", 0) + b.get("eyebrow_r", 0)) / 2.0 for b in recent]
        cur_brow = float(np.mean(heights))
        face_scale = max(20.0, buf[-1].get("face_scale", 50.0))
        # Rest baseline: only updates when brow is at rest (see update()); use 25th %ile if not set
        rest = getattr(self, "_brow_rest_baseline", 0.0)
        if rest <= 0 and len(buf) >= 4:
            all_h = [(b.get("eyebrow_l", 0) + b.get("eyebrow_r", 0)) / 2.0 for b in self._buf_tail(16)]
            rest = float(np.percentile(all_h, 25.0))
        if rest <= 0:
            rest = cur_brow
        raise_amt = (cur_brow - rest) / max(face_scale * 0.06, 1e-6)
        if raise_amt > 0.25:
            return 52.0 + min(45.0, raise_amt * 55.0)  # stays high while sustained
        if raise_amt > 0.08:
            return 38.0 + min(14.0, (raise_amt - 0.08) / 0.17 * 14.0)
        return 30.0
    ...
    def _g1_mouth_open_receptive(self, cur: dict, buf: list) -> float:
        ...
        mar = cur.get("mar", 0.2)
        if len(buf) >= 3:
            mar = float(np.median([b.get("mar", mar) for b in self._buf_tail(self._win(3))]))
        if 0.10 <= mar <= 0.28:
            return 35.0 + (mar - 0.10) / 0.18 * 55.0  # 35-90
        return 28.0
    ...
    def _g1_eye_widening(self, cur: dict, buf: list) -> float:
        ...
        r = area / base
        if r >= 1.08:
            return 55.0 + min(42.0, (r - 1.08) * 250.0)  # up to ~97
        if r >= 1.02:
            return 38.0 + (r - 1.02) / 0.06 * 22.0
        return 28.0
    ...
    def _g1_nod_intensity(self, buf: list) -> float:
        ...
        if len(buf) < 12:
            return 28.0
        pitches = [b["pitch"] for b in self._buf_tail(self._win(12))]
        pitch_std = float(np.std(pitches))
        d = np.diff(pitches)
        crosses = int(np.sum((d[:-1] * d[1:]) < 0))
        if 1 <= crosses <= 5 and pitch_std > 0.5:
            return 45.0 + min(52.0, pitch_std * 18.0 + crosses * 4.0)  # up to ~97
        return 25.0
    ...
    def _g1_rhythmic_nodding(self, buf: list) -> float:
        ...
        crosses = int(np.sum((d[:-1] * d[1:]) < 0))
        if pitch_std < 0.6:
            return 22.0 + pitch_std * 18.0
        nod_bonus = min(35.0, (crosses * 7.0) + (pitch_std - 0.6) * 25.0) if (1 <= crosses <= 4) else 0.0
        base = 38.0 + (pitch_std - 0.6) / 0.6 * 12.0 if pitch_std < 1.2 else 52.0 - (pitch_std - 1.2) * 8.0
        return float(max(20.0, min(100.0, base + nod_bonus)))
    ...
    def _g1_parted_lips(self, cur: dict, buf: list) -> float:
        ...
        if len(buf) >= 3:
            recent_mar = [b.get("mar", mar) for b in self._buf_tail(self._win(3))]
            mar = float(np.median(recent_mar))
        
        # Optimal band: receptive opening → high; closed or wide → low (full 0-100 span)
        if 0.12 <= mar <= 0.32:
            return 30.0 + (mar - 0.12) / 0.20 * 65.0  # 30-95
        if 0.08 <= mar < 0.12:
            return 22.0 + (mar - 0.08) / 0.04 * 18.0
        if 0.32 < mar <= 0.48:
            return 88.0 - (mar - 0.32) / 0.16 * 38.0  # ramp down for wide open
        return 20.0 + min(12.0, mar * 25.0)

    def _g1_softened_forehead(self, lm: np.ndarray, cur: dict, buf: list) -> float:
        """
        Softened forehead: default high (relaxed); score drops when brows furrowed, raised, or creases detected.
        All geometry from current-frame landmarks so the score updates every frame; buf only for brow-raise baseline.
        """
        lb = _safe(lm, LEFT_EYEBROW)
        rb = _safe(lm, RIGHT_EYEBROW)
        ...
        le = _safe(lm, LEFT_EYE)
        re = _safe(lm, RIGHT_EYE)
        # Face scale from current landmarks (inter-ocular) so we never rely on stale cur
        if len(le) >= 2 and len(re) >= 2:
            left_eye_c = np.mean(le[:, :2], axis=0)
            right_eye_c = np.mean(re[:, :2], axis=0)
            face_scale = float(np.linalg.norm(left_eye_c - right_eye_c))
        else:
        ...
        diff_normalized = (abs(inner_outer_diff_left) + abs(inner_outer_diff_right)) / max(face_scale * 0.10, 1e-6)
        evenness = max(0.0, 1.0 - min(1.0, diff_normalized))

        # Current brow height from current lm (eye Y - brow Y) so it updates every frame
        if len(le) and len(re):
            ley = float(np.mean(lb[:, 1]))
            rey = float(np.mean(rb[:, 1]))
            leye_y = float(np.mean(le[:, 1]))
            reye_y = float(np.mean(re[:, 1]))
            cur_brow = (leye_y - ley + reye_y - rey) / 2.0
        else:
            cur_brow = (cur.get("eyebrow_l", 0) + cur.get("eyebrow_r", 0)) / 2.0

        # Default high; add small per-frame variation when relaxed so the value is not stuck
        score = 86.0 + evenness * 6.0 + max(0.0, (0.015 - v_normalized) / 0.015 * 5.0)  # 86–97 when relaxed

        # Penalty 1: Furrow (inner brow corners pulled down)
        furrow_strength = 0.0
        if inner_outer_diff_left > 0 and inner_outer_diff_right > 0:
            avg_inner_lower = (inner_outer_diff_left + inner_outer_diff_right) / 2.0
            furrow_strength = min(1.0, avg_inner_lower / max(face_scale * 0.035, 1e-6))
        score -= furrow_strength * 55.0

        # Penalty 2: Brow raise (use buf for baseline; current brow from lm above)
        win = min(30, max(3, len(buf)))
        if len(buf) >= 2:
            recent_heights = [(b.get("eyebrow_l", 0) + b.get("eyebrow_r", 0)) / 2.0 for b in self._buf_tail(win)]
            baseline_brow = float(np.percentile(recent_heights, 25.0))
            raise_threshold = face_scale * 0.018
            if cur_brow > baseline_brow + raise_threshold:
                raise_amt = min(1.0, (cur_brow - baseline_brow) / max(face_scale * 0.055, 1e-6))
                score -= raise_amt * 42.0

        # Penalty 3: Creases / tension (variance and unevenness from current lm)
        crease_penalty = 0.0
        if v_normalized > 0.010:
            crease_penalty += min(25.0, (v_normalized - 0.010) / 0.025 * 25.0)
        if evenness < 0.85:
            crease_penalty += (1.0 - evenness) * 20.0
        score -= min(30.0, crease_penalty)
    ...
    def _g2_look_up_lr(self, cur: dict, buf: list) -> float:
        ...
        tasks; look-up-left (NLU) = visual/constructed imagery. Graduated scoring so
        minute shifts yield 48–55 instead of flat 45.
        """
        p = float(cur.get("pitch", 0))
        y = float(cur.get("yaw", 0))
        if len(buf) >= 3:
            recent_pitch = [b.get("pitch", p) for b in self._buf_tail(self._win(3))]
            recent_yaw = [b.get("yaw", y) for b in self._buf_tail(self._win(3))]
            p = float(np.median(recent_pitch))
            y = float(np.median(recent_yaw))
        pitch_deg = float(self._params.get("g2_look_up_pitch_deg", 3.0))
        yaw_deg = float(self._params.get("g2_look_up_yaw_deg", 5.0))
        looking_up = p > pitch_deg
        looking_lr = abs(y) > yaw_deg
        if looking_up and looking_lr:
            up_intensity = min(1.0, (p - pitch_deg) / 18.0)
            lr_intensity = min(1.0, (abs(y) - yaw_deg) / 28.0)
            combined = (up_intensity + lr_intensity) / 2.0
            return 52.0 + combined * 38.0
        if looking_up:
            up_intensity = min(1.0, (p - pitch_deg) / 12.0)
            return 50.0 + up_intensity * 28.0
        if looking_lr:
            lr_intensity = min(1.0, (abs(y) - yaw_deg) / 30.0)
            return 48.0 + lr_intensity * 16.0
        # Graduated: minute shifts above 0 give 45–48
        if p > 1 or abs(y) > 2:
            tiny = min(1.0, (max(0, p - 1) / 4.0 + max(0, abs(y) - 2) / 6.0) / 2.0)
            return 45.0 + tiny * 3.0
        return 45.0

    def _g2_lip_pucker(self, lm: np.ndarray, cur: dict, buf: list) -> float:
        """
        Lip Pucker: Detects pursed lips (thinking, evaluating, FACS-style pucker).
        
        Two pucker types: (1) "O" pucker = narrow mouth + round/moderate opening (MAR
        moderate-high). (2) Closed pucker = narrow mouth + lips together (low MAR).
        Previous logic required very high MAR and very low mw_normalized (e.g. < 0.12),
        which rarely occurred; thresholds are relaxed and both pucker types are scored.
        
        FALSE POSITIVE REDUCTION: Median of recent MAR/mouth width to avoid speech.
        """
        mar = cur.get("mar", 0.2)
        face_scale = max(cur.get("face_scale", 50.0), 20.0)
        mw = cur.get("mouth_w", 40.0)
        
        if len(buf) >= 3:
            recent_mar = [b.get("mar", mar) for b in self._buf_tail(self._win(3))]
            recent_mw = [b.get("mouth_w", mw) for b in self._buf_tail(self._win(3))]
            mar = float(np.median(recent_mar))
            mw = float(np.median(recent_mw))
        
        mw_normalized = mw / face_scale
        narrow = float(self._params.get("g2_lip_pucker_narrow", 0.30))
        very_narrow = 0.20
        neutral_mar_lo = 0.12
        neutral_mar_hi = 0.32
        if mw_normalized >= 0.35 and mar <= 0.30:
            return 12.0 + (mar / 0.22) * 10.0
        if mw_normalized >= narrow and mar <= neutral_mar_hi:
            return 18.0 + (mar / 0.28) * 12.0
        narrowness = 1.0 - (mw_normalized - very_narrow) / max(narrow - very_narrow, 0.01)
        narrowness = max(0.0, min(1.0, narrowness))
        if mar >= 0.22 and mar <= 0.55:
            round_factor = min(1.0, (mar - 0.22) / 0.20)
            pucker = 0.5 * narrowness + 0.5 * round_factor
            return 35.0 + pucker * 55.0
        if mar < neutral_mar_lo and mw_normalized < narrow:
            closed_factor = 1.0 - (mar / neutral_mar_lo)
            pucker = 0.6 * narrowness + 0.4 * closed_factor
            return 40.0 + pucker * 50.0
        if mw_normalized < narrow:
            mar_factor = max(0.0, (mar - 0.18) / 0.15) if mar > 0.18 else (1.0 - mar / 0.18) * 0.5
            pucker = 0.6 * narrowness + 0.4 * min(1.0, mar_factor)
            return 32.0 + pucker * 48.0
        # Soft ramp for moderate narrow (slight pucker): 25–40
        if mw_normalized < 0.34:
            mod_narrow = 1.0 - (mw_normalized - very_narrow) / max(0.34 - very_narrow, 0.01)
            mod_narrow = max(0.0, min(1.0, mod_narrow))
            return 22.0 + narrowness * 18.0 + mod_narrow * 8.0
        return 22.0 + narrowness * 18.0

    def _g2_eye_squint(self, cur: dict, buf: list) -> float:
        """
        Eye squint: FACS AU 7; cognitive load increases squinting. Continuous bands for
        slight (0.92–0.98), moderate (0.85–0.92), strong (<0.75). Blink excluded.
        """
        ear = cur.get("ear", 0.2)
        baseline_ear = self._baseline_ear if self._baseline_ear > 0.05 else 0.22
        if len(buf) >= 3:
            recent_ear = [b.get("ear", ear) for b in self._buf_tail(self._win(3)) if b.get("is_blink", 0) < 0.5]
            if recent_ear:
                ear = float(np.median(recent_ear))
        ear_ratio = ear / baseline_ear if baseline_ear > 0 else 1.0
        ratio_lo = float(self._params.get("g2_eye_squint_ratio_lo", 0.75))
        ratio_hi = float(self._params.get("g2_eye_squint_ratio_hi", 0.98))
        if ear_ratio < ratio_lo and ear < 0.15:
            return 68.0 + min(24.0, (ratio_lo - ear_ratio) * 120.0)
        if ear_ratio < 0.85 and ear < 0.17:
            return 52.0 + (0.85 - ear_ratio) * 160.0
        if ear_ratio < 0.92:
            return 52.0 + (0.92 - ear_ratio) / 0.07 * 16.0
        if ear_ratio < ratio_hi:
            return 40.0 + (ratio_hi - ear_ratio) / 0.06 * 12.0
        return 38.0

    def _g2_thinking_brow(self, lm: np.ndarray, buf: list) -> float:
        """
        Thinking brow: asymmetric brow raise; sustained = evaluation, doubt. Very slight
        band (0.015–0.020) gives 48–52 so minute asymmetry is visible.
        """
        lb = _safe(lm, LEFT_EYEBROW)
        rb = _safe(lm, RIGHT_EYEBROW)
        if len(lb) < 2 or len(rb) < 2:
        ...
        face_scale = max(20.0, face_scale)
        d_rel = d / face_scale
        min_rel = float(self._params.get("g2_thinking_brow_min_rel", 0.020))
        if len(buf) >= 3:
            recent_asymmetries = []
            for b in self._buf_tail(self._win(3)):
                bl = b.get("eyebrow_l", 0)
                br = b.get("eyebrow_r", 0)
                fs = b.get("face_scale", 50.0)
                if fs > 0:
                    recent_asymmetries.append(abs(bl - br) / max(fs, 20.0))
            high_asymmetry_count = sum(1 for a in recent_asymmetries if a > min_rel)
            if high_asymmetry_count < 2:
                return 44.0
        if 0.04 <= d_rel <= 0.18:
            return 68.0 + min(14.0, (d_rel - 0.04) * 100.0)
        if min_rel <= d_rel < 0.04:
            return 52.0 + (d_rel - min_rel) / max(0.04 - min_rel, 0.001) * 16.0
        if 0.015 <= d_rel < min_rel:
            return 48.0 + (d_rel - 0.015) / max(min_rel - 0.015, 0.001) * 4.0
        return 44.0

    def _g2_stillness(self, buf: list) -> float:
        """
        Stillness: low frame-to-frame facial movement; focused or frozen state. Very still
        band (mov_norm < g2_stillness_very_low_mov) reaches 88–95.
        """
        win = 10
        if len(buf) < win:
            return 45.0
        movements = [b.get("face_movement", 0.0) for b in self._buf_tail(win)]
        scales = [b.get("face_scale", 50.0) for b in self._buf_tail(win)]
        avg_scale = float(np.mean(scales)) if scales else 50.0
        scale_sq = max(400.0, avg_scale * avg_scale)
        mov_median = float(np.median(movements))
        mov_normalized = mov_median / scale_sq
        very_low = float(self._params.get("g2_stillness_very_low_mov", 0.00008))
        if mov_normalized < very_low:
            return 88.0 + (very_low - mov_normalized) / max(very_low, 1e-9) * 7.0
        if mov_normalized < 0.00015:
            return 82.0 + (0.00015 - mov_normalized) / 0.00015 * 14.0
        if mov_normalized < 0.0005:
            return 68.0 + (0.0005 - mov_normalized) / 0.00035 * 14.0
        if mov_normalized < 0.0012:
            return 54.0 + (0.0012 - mov_normalized) / 0.0007 * 14.0
        if mov_normalized < 0.003:
            return 38.0 + (0.003 - mov_normalized) / 0.0018 * 16.0
        if mov_normalized < 0.008:
            return 24.0 + (0.008 - mov_normalized) / 0.005 * 14.0
        return max(18.0, 38.0 - (mov_normalized - 0.008) * 1200.0)

    def _g2_gaze_shift_frequency(self, buf: list) -> float:
        """High shift rate = processing (Kahneman System 2). Low-shift band 0.5–1.5 gives 48–52."""
        if len(buf) < 10:
            return 50.0
        win = self._win(10)
        yaws = [b.get("yaw", 0) for b in self._buf_tail(win)]
        pitches = [b.get("pitch", 0) for b in self._buf_tail(win)]
        d_yaw = np.abs(np.diff(yaws))
        d_pitch = np.abs(np.diff(pitches))
        shift_rate = float(np.mean(d_yaw) + np.mean(d_pitch))
        if len(buf) >= 14:
            rates = []
            tail_list = self._buf_tail(len(buf))  # one list for sliding window
            for start in range(max(0, len(tail_list) - 12), len(tail_list) - win + 1):
                w = tail_list[start : start + win]
                if len(w) >= 2:
                    y = [b.get("yaw", 0) for b in w]
                    p = [b.get("pitch", 0) for b in w]
                    rates.append(float(np.mean(np.abs(np.diff(y))) + np.mean(np.abs(np.diff(p)))))
            if rates:
                shift_rate = float(np.median(rates))
        if shift_rate > 3.0:
            return 55.0 + min(35.0, shift_rate * 4.0)
        if shift_rate > 1.5:
            return 50.0 + (shift_rate - 1.5) * 5.0
        if shift_rate > 0.5:
            return 48.0 + (shift_rate - 0.5) * 4.0
        return 45.0

    def _g2_mouth_tight_eval(self, cur: dict, buf: list) -> float:
        """Tight mouth during evaluation. Gradient for slightly tight (baseline×0.75–0.90) = 42–55."""
        mar = cur.get("mar_inner", cur.get("mar", 0.2))
        if len(buf) >= 3:
            mar = float(np.median([b.get("mar_inner", b.get("mar", mar)) for b in self._buf_tail(self._win(3))]))
        baseline = max(0.08, self._baseline_mar) if self._baseline_mar > 0 else 0.18
        if mar >= baseline * 0.90:
            return 42.0
        ...
        return 42.0 + (0.90 - mar / baseline) * 13.0
    ...
    def _g3_contempt(
        ...
        recent_ok = 0
        if len(buf) >= 4:
            for b in self._buf_tail(self._win(4)):
    ...
    def _g3_nose_crinkle(self, cur: dict, buf: list) -> float:
        ...
        recent = [b.get("nose_height", 0.0) for b in self._buf_tail(self._win(6)) if b.get("nose_height", 0) > 0]
        if not recent:
            return 8.0
        ...
        short_count = sum(1 for b in self._buf_tail(self._win(4)) if (b.get("nose_height", 0) or 0) < med * 0.88)
    ...
    def _g3_lip_compression(self, cur: dict, buf: list) -> float:
        ...
        if buf_len >= 6:
            recent_mar = [b.get("mar_inner", b.get("mar", mar)) for b in self._buf_tail(self._win(6))]
            mar = float(np.median(recent_mar))
        ...
        below_count = sum(
            1 for b in self._buf_tail(self._win(6))
    ...
    def _g3_gaze_aversion(self, cur: dict, buf: list) -> float:
        ...
        recent_devs = []
        for b in self._buf_tail(n_recent):
    ...
    def _g3_no_nod(self, buf: list) -> float:
        ...
        pitches = [b["pitch"] for b in self._buf_tail(win)]
        pitch_std = float(np.std(pitches))
    ...
    def _g3_brow_lower_sustained(self, buf: list) -> float:
        ...
        if len(buf) < 8:
            return 35.0
        heights = [(b.get("eyebrow_l", 0) + b.get("eyebrow_r", 0)) / 2 for b in self._buf_tail(self._win(8))]
        face_scale = buf[-1].get("face_scale", 50.0)
    ...
    def _g3_eye_squeeze(self, cur: dict, buf: list) -> float:
        ...
        ear = cur.get("ear", 0.2)
        baseline_ear = self._baseline_ear if self._baseline_ear > 0.05 else 0.22
        if ear < 0.08 and (len(buf) < 4 or sum(1 for b in self._buf_tail(self._win(4)) if b.get("ear", 0.2) < 0.10) >= 2):
    ...
    def _g3_head_shake(self, buf: list) -> float:
        ...
        if len(buf) < 10:
            return 35.0
        yaws = [b.get("yaw", 0) for b in self._buf_tail(self._win(10))]
        d = np.diff(yaws)
        crosses = int(np.sum((d[:-1] * d[1:]) < 0))
    ...
    def _g4_relaxed_exhale(self, buf: list) -> float:
        ...
        if len(buf) < 8:
            return 48.0  # Warmup
        window_before = self._buf_tail(8)[:4]
        window_now = self._buf_tail(self._win(3))
        ...
        recent_mars = [b["mar"] for b in self._buf_tail(self._win(12))]
    ...
    def _g4_fixed_gaze(self, buf: list, w: int, h: int) -> float:
        ...
        window = self._buf_tail(self._win(14))
    ...
    def _g4_smile_transition(self, buf: list, duchenne: float) -> float:
        ...
        if len(buf) < 12:
            return 45.0
        mars = [b["mar"] for b in self._buf_tail(self._win(12))]
    ...
    def _g4_mouth_relax(self, buf: list) -> float:
        ...
        if len(buf) < 8:
            return 48.0
        mar_now = float(np.mean([b["mar"] for b in self._buf_tail(self._win(3))]))
        mar_before = float(np.mean([b["mar"] for b in self._buf_tail(8)[:4]]))
    ...
    def _g4_smile_sustain(self, buf: list, duchenne: float) -> float:
        ...
        if len(buf) < 12:
            return 45.0
        mar_mean = float(np.mean([b["mar"] for b in self._buf_tail(self._win(6))]))
        mar_std = float(np.std([b["mar"] for b in self._buf_tail(self._win(6))]))
...
DEFAULT_SIGNIFIER_WEIGHTS: List[float] = [
    ...
    1.05, 1.00, 1.00, 1.00, 1.10, 1.00, 1.00, 1.15, 0.50,
    1.10, 1.00, 1.15, 1.00, 1.05, 1.40, 1.10, 1.50, 1.00, 1.25,
    1.00, 1.55, 1.05, 0.50, 1.10, 1.10, 1.00, 1.00,
...
def compute_composite_metrics(
    ...
    """
    Compute 0-100 composite metrics for business meetings from facial (G1-G4), speech tags, and optional acoustic.

    Composites are tuned for meeting relevance: alignment, rapport, objections, decision readiness,
    confusion, disengagement risk, closing opportunity, and condition-based facial states
    (active listening, agreement/buy-in, resistance, closing window, etc.).
    ...
    decision_plus_voice, and condition-based facial composites.
    """
    out: Dict[str, float] = {}
    ...
    out["cognitive_processing_facial"] = float(np.clip(_condition_composite(sig, [("g2_stillness", 50.0, None), ("g2_gaze_shift_frequency", 48.0, None)], 70.0, 20.0), 0.0, 100.0))
...
def recommend_detection_method(
    ...
    Unified (MediaPipe + Azure) only when device has enough power and network is fast;
    low-end devices and slow network → MediaPipe only.
    """
    ...
    # Low tier (old computers, limited RAM) → MediaPipe only
    if tier == DEVICE_TIER_LOW:
        return "mediapipe"
    # Slow network/backend → MediaPipe only
    if azure_latency_ms is not None and azure_latency_ms > threshold:
        return "mediapipe"
    # High tier + good latency → use both (unified) when desired
    if tier == DEVICE_TIER_HIGH:
        return "unified" if return_unified_when_viable else "azure_face_api"
    # Medium tier: use both only if config allows; otherwise MediaPipe only to avoid overloading
    use_unified_only_high = getattr(config, "USE_UNIFIED_ONLY_FOR_HIGH_TIER", True)
    if use_unified_only_high:
        return "mediapipe"
...
MEDIUM_DROP_SIGNIFIERS: List[str] = [
    "g1_pupil_dilation", "g1_eyebrow_flash", "g1_facial_symmetry",
    "g1_micro_smile", "g1_brow_raise_sustained", "g1_eye_widening",
    "g2_stillness", "g2_gaze_shift_frequency",
    "g3_no_nod", "g3_narrowed_pupils", "g3_lip_corner_dip", "g3_brow_lower_sustained",
]
LOW_KEEP_SIGNIFIERS: List[str] = [
    "g1_duchenne", "g1_eye_contact", "g1_head_tilt", "g1_forward_lean",
    "g1_rhythmic_nodding", "g1_parted_lips", "g1_softened_forehead", "g1_mouth_open_receptive", "g1_nod_intensity",
    "g2_look_up_lr", "g2_eye_squint", "g2_thinking_brow", "g2_mouth_tight_eval",
...
def detect_opportunity(
    ...
    Args:
        ...
        signifier_scores: Optional 42 signifier scores.
...
def update_history_from_detector(group_means: Dict[str, float]) -> None:
    """Update internal history when caller uses external group_history (e.g. detector)."""
    _update_history(group_means)
