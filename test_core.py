...
# =============================================================================
# Synthetic landmarks for signifier/parameter sweep tests (inlined in this file)
# =============================================================================
...
def make_look_up_landmarks(shape: Tuple[int, int, int] = (480, 640, 3), pitch_offset: float = 25.0, yaw_offset: float = 0) -> np.ndarray:
    """Simulate look up (and optionally lateral): move nose up so pitch > 0; shift nose x for yaw."""
    h, w = shape[0], shape[1]
    lm = _make_base_landmarks(w, h)
    eye_cy = (np.mean([lm[i, 1] for i in LEFT_EYE + RIGHT_EYE if i < lm.shape[0]]))
    for idx in [NOSE_TIP] + [i for i in NOSE if i < lm.shape[0]]:
        lm[idx, 1] -= pitch_offset
        lm[idx, 0] += yaw_offset
    return lm


def make_squint_landmarks(shape: Tuple[int, int, int] = (480, 640, 3), ear_scale: float = 0.7) -> np.ndarray:
    """Narrow eyes to lower EAR (squint). ear_scale < 1 reduces eye opening."""
    lm = _make_base_landmarks(shape[1], shape[0])
    cy_leye = np.mean([lm[i, 1] for i in LEFT_EYE if i < lm.shape[0]])
    cy_reye = np.mean([lm[i, 1] for i in RIGHT_EYE if i < lm.shape[0]])
    for idx in LEFT_EYE:
        if idx < lm.shape[0]:
            lm[idx, 1] = cy_leye + (lm[idx, 1] - cy_leye) * (2.0 - ear_scale)
    for idx in RIGHT_EYE:
        if idx < lm.shape[0]:
            lm[idx, 1] = cy_reye + (lm[idx, 1] - cy_reye) * (2.0 - ear_scale)
    return lm


def make_lip_pucker_landmarks(shape: Tuple[int, int, int] = (480, 640, 3), mw_scale: float = 0.5, mar: float = 0.15) -> np.ndarray:
    """Narrow mouth (pucker) and set MAR. mw_scale < 1 narrows width; mar controls mouth height/width ratio."""
    lm = _make_base_landmarks(shape[1], shape[0])
    my = np.mean([lm[i, 1] for i in MOUTH if i < lm.shape[0]])
    cx = shape[1] / 2
    for idx in MOUTH + [MOUTH_LEFT, MOUTH_RIGHT]:
        if idx < lm.shape[0]:
            lm[idx, 0] = cx + (lm[idx, 0] - cx) * mw_scale
    mh = (np.max([lm[i, 1] for i in MOUTH if i < lm.shape[0]]) - np.min([lm[i, 1] for i in MOUTH if i < lm.shape[0]])) or 1
    mw = (np.max([lm[i, 0] for i in MOUTH if i < lm.shape[0]]) - np.min([lm[i, 0] for i in MOUTH if i < lm.shape[0]])) + 1e-6
    target_mh = mar * mw
    for idx in MOUTH:
        if idx < lm.shape[0]:
            lm[idx, 1] = my + (lm[idx, 1] - my) * (target_mh / max(mh, 1e-6))
    return lm


def make_thinking_brow_landmarks(shape: Tuple[int, int, int] = (480, 640, 3), asymmetry_px: float = 12.0) -> np.ndarray:
    """One brow higher than the other (asymmetric raise)."""
    lm = _make_base_landmarks(shape[1], shape[0])
    for idx in LEFT_EYEBROW:
        if idx < lm.shape[0]:
            lm[idx, 1] -= asymmetry_px
    return lm
...
class TestExpressionSignifierMetrics(unittest.TestCase):
    ...
    def test_g2_look_up_lr_higher_when_looking_up(self):
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_look_up_landmarks(self.shape, pitch_offset=30, yaw_offset=15), None, self.shape)
        scores_look = self.engine.get_all_scores()
        self.engine.reset()
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_neutral_landmarks(self.shape), None, self.shape)
        scores_neutral = self.engine.get_all_scores()
        self.assertGreater(scores_look["g2_look_up_lr"], scores_neutral["g2_look_up_lr"])

    def test_g2_lip_pucker_higher_for_puckered_lips(self):
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_lip_pucker_landmarks(self.shape, mw_scale=0.45, mar=0.12), None, self.shape)
        scores_pucker = self.engine.get_all_scores()
        self.engine.reset()
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_neutral_landmarks(self.shape), None, self.shape)
        scores_neutral = self.engine.get_all_scores()
        self.assertGreater(scores_pucker["g2_lip_pucker"], scores_neutral["g2_lip_pucker"])

    def test_g2_eye_squint_higher_for_squinted_eyes(self):
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_squint_landmarks(self.shape, ear_scale=0.72), None, self.shape)
        scores_squint = self.engine.get_all_scores()
        self.engine.reset()
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_neutral_landmarks(self.shape), None, self.shape)
        scores_neutral = self.engine.get_all_scores()
        self.assertGreater(scores_squint["g2_eye_squint"], scores_neutral["g2_eye_squint"])

    def test_g2_thinking_brow_higher_for_asymmetric_brow(self):
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_thinking_brow_landmarks(self.shape, asymmetry_px=14), None, self.shape)
        scores_thinking = self.engine.get_all_scores()
        self.engine.reset()
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_neutral_landmarks(self.shape), None, self.shape)
        scores_neutral = self.engine.get_all_scores()
        self.assertGreater(scores_thinking["g2_thinking_brow"], scores_neutral["g2_thinking_brow"])

    def test_g2_stillness_higher_when_face_unchanged(self):
        neutral = make_neutral_landmarks(self.shape)
        warmup_engine(self.engine, neutral, self.shape, 15)
        for _ in range(12):
            self.engine.update(neutral, None, self.shape)
        scores_still = self.engine.get_all_scores()
        self.engine.reset()
        warmup_engine(self.engine, neutral, self.shape, 15)
        for i in range(12):
            lm = make_neutral_landmarks(self.shape)
            if i % 2 == 0:
                for j in range(lm.shape[0]):
                    lm[j, 0] += 2.0 * (i % 3)
                    lm[j, 1] += 1.5
            self.engine.update(lm, None, self.shape)
        scores_moving = self.engine.get_all_scores()
        self.assertGreater(scores_still["g2_stillness"], scores_moving["g2_stillness"])

    def test_g2_mouth_tight_eval_higher_for_tight_mouth(self):
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_lip_compression_landmarks(self.shape), None, self.shape)
        scores_tight = self.engine.get_all_scores()
        self.engine.reset()
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(8):
            self.engine.update(make_neutral_landmarks(self.shape), None, self.shape)
        scores_neutral = self.engine.get_all_scores()
        self.assertGreater(scores_tight["g2_mouth_tight_eval"], scores_neutral["g2_mouth_tight_eval"])

    def test_g2_gaze_shift_frequency_higher_when_gaze_varies(self):
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for i in range(12):
            yaw_off = 25.0 * (1 if i % 2 == 0 else -1)
            self.engine.update(make_gaze_aversion_landmarks(self.shape, yaw_offset=yaw_off), None, self.shape)
        scores_shift = self.engine.get_all_scores()
        self.engine.reset()
        warmup_engine(self.engine, make_neutral_landmarks(self.shape), self.shape, 15)
        for _ in range(12):
            self.engine.update(make_neutral_landmarks(self.shape), None, self.shape)
        scores_neutral = self.engine.get_all_scores()
        self.assertGreater(scores_shift["g2_gaze_shift_frequency"], scores_neutral["g2_gaze_shift_frequency"])
